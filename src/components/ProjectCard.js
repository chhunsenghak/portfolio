...
export default function ProjectCard({
  ...
  const st = {
    card: {
      ...
      boxShadow: hov
        ? `0 16px 40px rgba(0,0,0,.4), 0 0 0 1px ${meta.color}22`
        : "none",
      height: "100%",
    },
    stripe: {
  ...
  return (
    ...
    >
      ...
      {shine.on && (
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: `radial-gradient(circle at ${shine.x}px ${shine.y}px,rgba(255,255,255,.05),transparent 60%)`,
            pointerEvents: "none",
          }}
        />
      )}
      ...
      <div style={st.actions}>
        {live && (
          <LinkBtn href={live} accent={meta.color}>
            Live Demo
          </LinkBtn>
        )}
        {github && (
          <LinkBtn href={github} accent={meta.color}>
            GitHub
          </LinkBtn>
        )}
      </div>
...
function LinkBtn({ children, href, accent = C.accent }) {
  return (
    <motion.a
      href={href}
      target="_blank"
